...
function handlers() {
  ...
  document.getElementById("new_question").onclick = function () {
    reload();
  };
...
function randomise() {
  ...
  for (var i = 0; i < selection_artefact.numOfCards; i++) {
    ...
    if ((i + 1) % 12 == 0)
      classToFill.appendChild(document.createElement("br"));
  }
}
...
function compare(i, j) {
  ...
  for (
    var n = selection_artefact.iterator1;
    n < selection_artefact.numOfCards;
    n++
  ) {
    if (n == j) selection_artefact.card[n].style.backgroundColor = "#a4c652";
    else if (n == i) selection_artefact.card[n].style.backgroundColor = "black";
    else selection_artefact.card[n].style.backgroundColor = "#288ec8";
  }
  if (
    eval(selection_artefact.card[j].innerHTML) <
    eval(selection_artefact.card[i].innerHTML)
  )
    return true;
  else return false;
}
...
function swap(i, j) {
  ...
  selection_artefact.card[j].innerHTML = eval(
    selection_artefact.card[i].innerHTML
  );
...
function selection() {
  if (
    selection_artefact.iterator2 >= selection_artefact.numOfCards - 1 &&
    !selection_artefact.finished
  ) {
    ...
    document.getElementById("ins").innerHTML =
      "Iterations completed: " + selection_artefact.counter;
  }
  if (selection_artefact.iterator2 < selection_artefact.numOfCards - 1)
    ...
    if (selection_artefact.iterator1 == selection_artefact.numOfCards - 1) {
      ...
      selection_artefact.interval = 0;
      document.getElementById("ins").innerHTML =
        "<h3>The sort is complete - there were " +
        selection_artefact.comparisons +
        " comparisons and " +
        selection_artefact.swaps +
        " swaps.</h3>";
...
function next_step() {
  ...
  if (selection_artefact.action == 1) {
    ...
    else window[selection_artefact.fn_name]();
  } else {
    selection_artefact.action = 1;
...
function start_sort() {
  ...
  selection_artefact.card = document.querySelectorAll(".card");
  document.getElementById("comment-box-smaller-exercise").style.visibility =
    "visible";
  selection_artefact.action = 1;
  selection_artefact.finished = true;
...
function checkAnswer() {
  ...
  if (text1 == "" || text2 == "")
    ...
    text1 == selection_artefact.answer1 &&
    text2 == selection_artefact.answer2
  ) {
...
function makeQuestion() {
  var k = Math.floor(Math.random() * 6) + 2;
  let suffix = "th";
  if (k === 2) suffix = "nd";
  else if (k === 3) suffix = "rd";
  else if (k === 1) suffix = "st";
  // Clear, correct question text:
  document.getElementById(
    "question"
  ).innerHTML = `<b>Question :</b> Find the difference between the ${k}${suffix} smallest element and the smallest element in the following array. Report both the difference and the number of iterations performed by Selection Sort to place the ${k}${suffix} smallest element in its correct position. (Hint: Each iteration places the next smallest element in order.)`;
  selection_artefact.copy.sort();
  selection_artefact.answer1 =
    selection_artefact.copy[k - 1] - selection_artefact.copy[0];
